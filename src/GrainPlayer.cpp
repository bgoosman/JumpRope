...
void GrainPlayer::updatePlayHead(){
    if(!bSetPosition) {
        if(bUpdatePlayheadEvent==true){
            ps->setPosition(ofMap(samp.getRecordPosition(),0,LENGTH,0.0,1.0));
        }
        bUpdatePlayheadEvent = false;
    } else {
...
float GrainPlayer::getRecordPostion(){
    recordPosition = ofMap(samp.getRecordPosition(), 0,LENGTH,0.0,1.0);
...
void GrainPlayer::drawWaveform(){
    ...
    float sampRecPerc = (float)samp.getRecordPosition() / (float)LENGTH;
