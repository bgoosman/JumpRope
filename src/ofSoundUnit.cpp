...
ofDspChainSoundBuffer::ofDspChainSoundBuffer() { 
	numFrames = 0; 
	numChannels = 0; 
	buffer = NULL; 
}
ofDspChainSoundBuffer::ofDspChainSoundBuffer( const ofDspChainSoundBuffer& other ) { 
	copyFrom( other ); 
}
ofDspChainSoundBuffer::ofDspChainSoundBuffer( int nFrames, int nChannels ) { 
	numFrames = nFrames; 
	numChannels = nChannels; 
	buffer = new float[nFrames*nChannels]; 
}
ofDspChainSoundBuffer& ofDspChainSoundBuffer::operator=( const ofDspChainSoundBuffer& other ) { 
	copyFrom( other ); 
	return *this; 
}
...
void ofDspChainSoundBuffer::copyFrom( const ofDspChainSoundBuffer& other ) { 
...
/// Set audio data to 0.
void ofDspChainSoundBuffer::clear() { 
	if ( buffer ) { 
...
void ofDspChainSoundBuffer::allocate( int nFrames, int nChannels ) { 
...
void ofDspChainSoundBuffer::copyChannel( int channel, float* output ) const {
...
void ofDspChainSoundBuffer::copyTo( float* outBuffer, int outNumFrames, int outNumChannels ) {
	if ( numFrames == 0 || numChannels == 0 ) {
		ofLog( OF_LOG_WARNING, "ofDspChainSoundBuffer::copyTo: copy requested on empty buffer, returning nothing (check your addInputTo() methods!)" );
		memset( outBuffer, 0, sizeof(float)*outNumFrames*outNumChannels );
	}
	if ( outNumFrames>numFrames ||  outNumChannels>numChannels ) {
		ofLog( OF_LOG_WARNING, "ofDspChainSoundBuffer::copyTo: %i frames requested but only %i	are available", outNumFrames, numFrames );
		ofLog( OF_LOG_WARNING, "ofDspChainSoundBuffer::copyTo: %i channels requested but only %i are available, looping to make up the difference", outNumChannels, numChannels );
